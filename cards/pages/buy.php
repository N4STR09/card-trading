...
if (isset($_GET['id'])) {
    ...
    if ($card = mysqli_fetch_assoc($card_result)) {
        ...
        $message = "Error: Esta carta ya ha sido vendida o no existe.";
    }
}
        ...
        <table>
            ...
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td>
                        <img src="<?php echo htmlspecialchars($row['image_url']); ?>" alt="<?php echo htmlspecialchars($row['name']); ?>" width="50">
                        <?php echo htmlspecialchars($row['name']); ?>
                    </td>
                    <td><?php echo htmlspecialchars($row['rarity']); ?></td>
                    <td>$<?php echo number_format($row['price'], 2); ?></td>
                    <td>
                        <a href="buy.php?id=<?php echo $row['id']; ?>">Comprar</a>
                    </td>
                </tr>
            <?php } ?>
        </table>
